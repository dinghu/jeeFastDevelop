...
<#macro button type="info" size='xs' icon='' onclick='' disabled='' bigger='125' funcCode=''>
    ...
    <#if tabId?if_exists && ''!=funcCode>
        <#if SecurityCache.hasPrivilege(Request,tabId,funcCode)>
            <button type="button" class="btn btn-${type} btn-${size}" <#if onclick != ''>onclick="${onclick}"
                    </#if><#if disabled != ''>disabled="${disabled}"</#if>>
...
<#macro formgroup  inc=true type='text' title='title' name='name' value='' widthleft=3 widthdata=5>
    <#if inc>
        <@row>
            <div class="form-group">
                <label class="control-label col-xs-${widthleft} col-sm-${widthleft} col-md-${widthleft} col-lg-${widthleft}">${title}
                    ：</label>
    ...
    <#else>
        <@row>
            <div class="form-group">
                <label for="id-${name}"
                       class="control-label col-xs-${widthleft} col-sm-${widthleft-1} col-md-${widthleft-1} col-lg-${widthleft-1}">${title}
                    ：</label>
                <div class="col-xs-${12-widthleft} col-sm-${12-widthleft+1} col-md-${12-widthleft+1} col-lg-${12-widthleft+1}">
                    <input type="${type}" name="${name}" value="${value}" placeholder="${title}" class="form-control"
                           id="id-${name}">
                </div>
            </div>
        </@row>
...
<#macro date name dateValue=.now?date required="" default=false>
    <div class="input-group">
        <#if default>
            <input name="${name}" class="date form-control date-picker" type="text" check-type="${required}"
                   value="${dateValue?string('yyyy-MM-dd')}" readonly="readonly" placeholder="请点击选择日期"/>
        <#else>
            <input name="${name}" class="date form-control date-picker" type="text" check-type="${required}"
                   value="<#if dateValue?string('yyyy-MM-dd')!=.now?date?string('yyyy-MM-dd')>${dateValue?date?string('yyyy-MM-dd')}<#else></#if>"
                   readonly="readonly" placeholder="请点击选择日期"/>
        </#if>
...
<#macro select name list value='' listKey='' listValue='' headerKey='请选择' headerValue='' emptyOption=true id='' class='' style='' title='' disabled=false size=-1 tabindex=-1 onChange='' checkType='' sortBy='' multi=false search=true>
    <select <#if multi>multiple="multiple"</#if> name="${name}" <#if id?exists && id!=''>id="${id}"</#if>
            class="${class}" <#if style?exists && style!=''>style="${style}"</#if> <#if checkType?exists && checkType!=''>check-type="${checkType}"</#if> <#if title?exists && title!=''>title="${title}"</#if> <#if size gt 0> size="${size}"</#if> <#if disabled>disabled="disabled"</#if> <#if tabindex gt 0>tabindex="${tabindex}"</#if> <#if onChange?exists && onChange!=''>onChange='${onChange}'</#if>
            <#if value?exists>init-data=<@array2str array=value/></#if>>
            ...
            <#if listKey?exists && listKey!='' && listValue?exists && listValue!=''>
                <#list list?if_exists?sort_by(sortBy) as item>
                    ...
                    <option value="${itemValue}"
                            <#if equal(value,itemValue)>selected="selected"</#if> >${itemTitle}</option>
                </#list>
        ...
        <#else>
            <#if listKey?exists && listKey!='' && listValue?exists && listValue!=''>
                <#list list?if_exists as item>
                    ...
                    <option value="${itemValue}"
                            <#if equal(value,itemValue)>selected="selected"</#if> <#if extra!=''>extra='${itemExtra}'</#if>>${itemTitle}</option>
                </#list>
            <#elseif list?is_hash&&!list?is_collection>
