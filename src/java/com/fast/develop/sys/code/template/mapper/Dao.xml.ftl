...
<mapper namespace="${model.packageForDAO}.${model.name?cap_first}Dao">
    ...
    <insert id="save" useGeneratedKeys="true" keyProperty="id" keyColumn="id"
            parameterType="${model.packageForModel}.${model.name?cap_first}Entity">
        insert into ${model.tableName}
        (<#list model.fields as field>${field.columnName}<#if field_index+1!=model.fields?size>,</#if></#list>)
    ...
    <select id="getCount" resultType="java.lang.Integer">
        ...
        <include refid="com.fast.develop.framework.dao.CommonDao.queryParam"/>
    </select>

    <select id="findList" resultType="${model.name?cap_first}Entity">
        SELECT * FROM ${model.tableName}
        <include refid="com.fast.develop.framework.dao.CommonDao.queryParam"/>
        <include refid="com.fast.develop.framework.dao.CommonDao.queryOrder"/>
    </select>
