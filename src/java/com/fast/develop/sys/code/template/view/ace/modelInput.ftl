...
<#list model.fields as field>
    <#if field.propName!="id">
        <${'@'}formgroup title='${field.columnComment}'>
            ...
            <${'@'}date name="${field.propName?uncap_first}"
                        dateValue=${model.name?uncap_first}.${field.propName?uncap_first} <#if field.required>required="required"</#if>
                        default=true/>
                <#else>
                    <input type="text" name="${field.propName?uncap_first}"
                           value="${'$'}{${model.name?uncap_first}.${field.propName?uncap_first}}"
                           <#if field.checkType!="">check-type="${field.checkType}"</#if>>
                </#if>
        </${'@'}formgroup>
    </#if>
</#list>
